...
#### 方式 1：在请求头中传入 API Key

```
Authorization: Bearer YOUR_API_KEY
```

#### 方式 2：在 `Sec-WebSocket-Protocol` 中传入 Key

适用于部分 OpenAI Realtime 兼容客户端：

```
Sec-WebSocket-Protocol: realtime, openai-insecure-api-key.YOUR_API_KEY
```

不要使用 `?api_key=` URL 参数方式。
...
<Warning>
  Realtime API 使用 WebSocket 长连接，请确保客户端支持 WebSocket 协议。请优先使用请求头认证，不要依赖 `?api_key=` 查询参数。
</Warning>
