...
<Steps>
  <Step title="创建 n8n 专用 token">
    在 Crazyrouter 后台创建一个名为 `n8n` 的 token。首次建议只开放 `gpt-5.5` 和 `claude-sonnet-4-6`。
  </Step>
...
- [ ] `OpenAI API` 凭证已成功保存
- [ ] 节点级 `Base URL` 已设置为 `https://cn.crazyrouter.com/v1`
...
| 404 | `Base URL` 填成根域名或完整接口路径 | 改成 `https://cn.crazyrouter.com/v1` |
