...
- 默认国际 API Base URL：`https://api.crazyrouter.com`
- 认证变量：`ANTHROPIC_API_KEY`
- 推荐默认模型：`claude-opus-4-8`

<Tip>
Claude Code 自己会补完整的 Anthropic 请求路径，所以 Base URL 必须写根域名 `https://api.crazyrouter.com` 或 `https://api.crazyrouter.com`，不要手动拼 `/v1` 或 `/v1/messages`。
</Tip>
...
<Steps>
  ...
  <Step title="第 5 步：先在当前终端设置临时环境变量">
    ...
    把 `ANTHROPIC_BASE_URL` 写成根域名 `https://api.crazyrouter.com`，不要写成 `https://api.crazyrouter.com/v1`。
  </Step>
  ...
  <Step title="第 6 步：把环境变量写入持久配置">
    ...
    持久变量中的 `ANTHROPIC_BASE_URL` 也应写成 `https://api.crazyrouter.com`。
...
- [ ] `node -v` 不低于 18
- [ ] `claude --version` 正常
- [ ] `ANTHROPIC_BASE_URL` 已设置为 `https://api.crazyrouter.com` 或 `https://api.crazyrouter.com`
...
| 404 | Base URL 写成 `/v1`、`/v1/messages` 等错误路径 | 改回 `https://api.crazyrouter.com` 或 `https://api.crazyrouter.com` |
...
写根域名：`https://api.crazyrouter.com`

默认国际 API 入口写根域名：`https://api.crazyrouter.com`
