...
- 推荐默认模型：`claude-opus-4-8`

<Tip>
Claude Code 自己会补完整的 Anthropic 请求路径，所以 Base URL 必须写根域名 `https://cn.crazyrouter.com` 或 `https://cn.crazyrouter.com`，不要手动拼 `/v1` 或 `/v1/messages`。
</Tip>
  ...
  <Step title="第 6 步：把环境变量写入持久配置">
    ...
    <Tabs>
      <Tab title="Linux Bash">
        ```bash
        echo 'export ANTHROPIC_BASE_URL=https://cn.crazyrouter.com' >> ~/.bashrc
      ...
      <Tab title="macOS / Zsh">
        ```bash
        echo 'export ANTHROPIC_BASE_URL=https://cn.crazyrouter.com' >> ~/.zshrc
      ...
      <Tab title="Windows PowerShell">
        ```powershell
        [System.Environment]::SetEnvironmentVariable("ANTHROPIC_BASE_URL", "https://cn.crazyrouter.com", "User")
...
建议先把默认工作流稳定在 `claude-opus-4-8`，只有在确实遇到复杂任务时再切 `claude-opus-4-8`。
...
> 上述测试基于 Claude Code 真实客户端，经 `https://cn.crazyrouter.com` 根域名走原生 `POST /v1/messages` 链路验证。
...
- [ ] `node -v` 不低于 18
- [ ] `claude --version` 正常
- [ ] `ANTHROPIC_BASE_URL` 已设置为 `https://cn.crazyrouter.com` 或 `https://cn.crazyrouter.com`
...
| 404 | Base URL 写成 `/v1`、`/v1/messages` 等错误路径 | 改回 `https://cn.crazyrouter.com` 或 `https://cn.crazyrouter.com` |
...
先用 `claude-opus-4-8`。它通常是最稳的基线模型。
