...
<Note>
Dify 官方文档把自定义 `API Base URL` 视为代理或兼容上游场景下的可选项；对接 Crazyrouter 时，就应把 Crazyrouter 当作这个 OpenAI 兼容上游来填写 `https://cn.crazyrouter.com/v1`。另外，模型供应商通常需要工作区管理员或 Owner 权限才能配置；如果你看不到入口，先检查当前账号角色。
</Note>
...
- [ ] `API Base URL` 已设置为 `https://cn.crazyrouter.com/v1`
- [ ] 至少一个 LLM 已成功添加
- [ ] 第一个 Dify 应用请求成功返回
...
| 404 | `API Base URL` 写成根域名或完整接口路径 | 改成 `https://cn.crazyrouter.com/v1` |
