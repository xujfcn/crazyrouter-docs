...
- 请求方法：`POST`
- URL：`https://cn.crazyrouter.com/v1/chat/completions`
- Header：
...
| 使用场景 | 推荐模型 | 原因 |
|----------|----------|------|
| 首轮链路验证 | `gpt-5.5` | 2026 年 3 月 23 日已在生产环境实测成功，最适合先确认 Coze 到 Crazyrouter 的调用链路 |
| 高质量输出 | `claude-opus-4-8` | 适合更强解释和内容生成 |
...
- [ ] 请求 URL 已设置为 `https://cn.crazyrouter.com/v1/chat/completions`
- [ ] `Authorization` 头已正确填写 `Bearer sk-xxx`
- [ ] `Content-Type` 已设置为 `application/json`
- [ ] 最小请求体已用 `gpt-5.5` 验证成功
- [ ] 插件或节点单独调试成功
- [ ] 再挂入 Bot / 工作流后也能成功运行
...
| `model not found` | 模型名写错 | 先改回 `gpt-5.5` 重新验证 |
...
填 `https://cn.crazyrouter.com/v1/chat/completions`。
