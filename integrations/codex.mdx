...
保持 `base_url = "https://cn.crazyrouter.com/v1"` 和 `wire_api = "responses"` 不变，只替换 `model` 字段即可。例如：
...
- [ ] `base_url` 已设置为 `https://cn.crazyrouter.com/v1`
...
| 404 | `base_url` 写错，或漏了 `/v1` | 改成 `https://cn.crazyrouter.com/v1` |
...
写成 `https://cn.crazyrouter.com/v1`。
