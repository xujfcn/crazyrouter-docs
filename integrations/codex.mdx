...
| Claude 风格替代 | `claude-sonnet-4-6` | 更适合长文解释、复杂总结和多轮协作 |
| Gemini 备用档 | `gemini-3-pro-preview` | 适合作为第二条兼容性验证路径 |

推荐顺序：先让 `gpt-5.4` 跑通；之后再按场景补 `claude-sonnet-4-6` 或 `gemini-3-pro-preview`。
...
- 默认主力建议 `gpt-5.4`，如需交叉验证再补 `claude-sonnet-4-6`
