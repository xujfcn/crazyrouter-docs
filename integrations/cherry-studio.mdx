...
<Steps>
  ...
  <Step title="拉取并添加模型">
    点击 `管理` 或 `Manage` 拉取模型列表，然后手动把需要的模型加入该服务商。注意：弹窗里出现模型不代表已经加入可用列表，通常还需要再点一次 `+`。首次建议只加一个基线模型：`gpt-5.4`。
  </Step>
  <Step title="开启服务商并做首次对话">
    打开右上角启用开关，回到聊天界面选择 `gpt-5.4`，发送一句简单请求，例如：`Reply only OK`。收到正常回复后，再开始添加其他模型。
  </Step>
</Steps>
...
推荐顺序：先让 `gpt-5.4` 跑通，再逐步加入 `claude-sonnet-4-6` 和 `gemini-3-pro`。
...
- 默认先用 `gpt-5.4`，复杂长文和代码解释再切 `claude-sonnet-4-6`
