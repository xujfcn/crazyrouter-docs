...
```text
OPENAI_API_BASE=https://cn.crazyrouter.com/v1
```

同时要注意：Crazyrouter 的模型名就是后台模型列表和 pricing 页面展示的原始模型 ID，例如 `gpt-5.5`、`claude-sonnet-4-6`。在 Aider 里也应直接填写这个模型 ID：
...
<Warning>
不要给 Crazyrouter 模型名额外添加 `openai/` 前缀。`openai/gpt-5.5` 不是本站模型名，会导致 `model not found` 或路由无可用渠道。
</Warning>
...
<Steps>
  ...
  <Step title="第 5 步：先在当前终端设置临时环境变量">
    ...
    <Tabs>
      <Tab title="macOS / Linux">
        ...
        export OPENAI_API_BASE=https://cn.crazyrouter.com/v1
        echo $OPENAI_API_KEY
      ...
      <Tab title="Windows PowerShell">
        ...
        $env:OPENAI_API_BASE = "https://cn.crazyrouter.com/v1"
  ...
  <Step title="第 6 步：把环境变量写入持久配置">
    <Tabs>
      <Tab title="Linux Bash">
        ...
        echo 'export OPENAI_API_BASE=https://cn.crazyrouter.com/v1' >> ~/.bashrc
        source ~/.bashrc
        echo $OPENAI_API_BASE
        ```
      </Tab>
      <Tab title="macOS / Zsh">
        ...
        echo 'export OPENAI_API_BASE=https://cn.crazyrouter.com/v1' >> ~/.zshrc
        source ~/.zshrc
        echo $OPENAI_API_BASE
        ```
      </Tab>
      <Tab title="Windows PowerShell">
        ...
        [System.Environment]::SetEnvironmentVariable("OPENAI_API_BASE", "https://cn.crazyrouter.com/v1", "User")
  ...
  <Step title="第 7 步：可选写入 ~/.aider.conf.yml">
    ...
    <Tabs>
      <Tab title="macOS / Linux">
        ...
        model: gpt-5.5
        openai-api-base: https://cn.crazyrouter.com/v1
        EOF
        cat ~/.aider.conf.yml
        ```
      </Tab>
  ...
  <Step title="第 9 步：启动 Aider 并完成第一次验证">
    ...
    <Tip>
    如果你临时想验证另一个模型，写法也保持同样模式，例如 `aider --model claude-sonnet-4-6`。
    </Tip>
  </Step>
</Steps>
...
| Claude 风格替代 | `claude-sonnet-4-6` | 适合长上下文解释和较稳的多轮协作 |
| Gemini 备用档 | `gemini-3-pro` | 适合作为第二条兼容性验证路径 |

建议先用 `gpt-5.5`，跑通后再视场景增加 `claude-sonnet-4-6` 或 `gemini-3-pro`。
...
- [ ] `OPENAI_API_BASE` 已设置为 `https://cn.crazyrouter.com/v1`
- [ ] `~/.aider.conf.yml` 如有使用，模型名写法为 Crazyrouter 原始模型 ID，例如 `gpt-5.5`
- [ ] `aider --model gpt-5.5` 可以正常启动
- [ ] 第一个只读或小改动任务成功返回
...
| 404 | Base URL 填错，或少了 `/v1` | 改成 `https://cn.crazyrouter.com/v1` |
| `model not found` | 模型名写错、使用了不存在的 `openai/` 前缀，或该模型当前未在本站开放 | 改回 pricing 页面或模型列表中存在的原始模型 ID，例如 `gpt-5.5`、`claude-sonnet-4-6` |
...
- 第一次接入时，先在小仓库里验证
- 默认用 `gpt-5.5`，如需交叉验证再加 `claude-sonnet-4-6`
...
### 为什么模型名不要写成 `openai/gpt-5.5`？

因为 Crazyrouter 的模型名不带供应商前缀，本站实际识别的是 `gpt-5.5` 这类原始模型 ID。`openai/gpt-5.5` 会被当作另一个模型名处理，本站没有这个模型。
