...
同时要注意：Crazyrouter 的模型名就是后台模型列表和 pricing 页面展示的原始模型 ID，例如 `gpt-5.4`、`claude-sonnet-4-6`。在 Aider 里也应直接填写这个模型 ID：
...
<Warning>
不要给 Crazyrouter 模型名额外添加 `openai/` 前缀。`openai/gpt-5.4` 不是本站模型名，会导致 `model not found` 或路由无可用渠道。
</Warning>
...
| 使用场景 | Aider 中建议写法 | 原因 |
|----------|------------------|------|
| 默认主力编码 | `gpt-5.4` | 使用 Crazyrouter 原始模型 ID，适合作为 Aider OpenAI 兼容基线 |
| Claude 风格替代 | `claude-sonnet-4-6` | 适合长上下文解释和较稳的多轮协作 |
| Gemini 备用档 | `gemini-3-pro` | 适合作为第二条兼容性验证路径 |

建议先用 `gpt-5.4`，跑通后再视场景增加 `claude-sonnet-4-6` 或 `gemini-3-pro`。
...
- [ ] `~/.aider.conf.yml` 如有使用，模型名写法为 Crazyrouter 原始模型 ID，例如 `gpt-5.4`
- [ ] `aider --model gpt-5.4` 可以正常启动
- [ ] 第一个只读或小改动任务成功返回
...
| `model not found` | 模型名写错、使用了不存在的 `openai/` 前缀，或该模型当前未在本站开放 | 改回 pricing 页面或模型列表中存在的原始模型 ID，例如 `gpt-5.4`、`claude-sonnet-4-6` |
...
- 第一次接入时，先在小仓库里验证
- 默认用 `gpt-5.4`，如需交叉验证再加 `claude-sonnet-4-6`
...
### 为什么模型名不要写成 `openai/gpt-5.4`？

因为 Crazyrouter 的模型名不带供应商前缀，本站实际识别的是 `gpt-5.4` 这类原始模型 ID。`openai/gpt-5.4` 会被当作另一个模型名处理，本站没有这个模型。
