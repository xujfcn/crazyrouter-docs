...
- 服务类型：`OpenAI`
- API Key：你的 `sk-...`
- 自定义 API 地址：`https://cn.crazyrouter.com/v1/chat/completions`
- 推荐首个验证模型：`gpt-5.5`
...
<Steps>
  ...
  <Step title="第 4 步：填写 API Key 和自定义地址">
    ...
    - `API Key`：你的 `sk-...`
    - `自定义 API 接口地址`：`https://cn.crazyrouter.com/v1/chat/completions`
    - `模型`：先填 `gpt-5.5`
...
- [ ] 已选择 `OpenAI` 翻译服务
- [ ] `API Key` 已填写正确
- [ ] `自定义 API 接口地址` 已设置为 `https://cn.crazyrouter.com/v1/chat/completions`
...
| 现象 | 常见原因 | 修复方式 |
|------|----------|----------|
| 404 | API 地址少写了 `/chat/completions`，或写成了根域名 | 改成完整地址 `https://cn.crazyrouter.com/v1/chat/completions` |
...
填完整路径：`https://cn.crazyrouter.com/v1/chat/completions`。
