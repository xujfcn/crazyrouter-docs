...
| 场景 | 推荐模型 | 原因 |
|------|----------|------|
| 首轮链路验证 | `gpt-4.1` | 最适合先验证插件到 Crazyrouter 的基础链路 |
| 文献翻译 | `gpt-4.1-mini` | 成本更低，适合较频繁翻译 |
...
| `model not found` | 模型名写错 | 先改回 `gpt-4.1` 做最小验证 |
...
<Note>
如果你的目标是“先让 Zotero 跑通 Crazyrouter”，最关键的不是一开始就追求最强插件，而是先确认你选的插件真的支持自定义 OpenAI 兼容地址，然后用 `gpt-4.1` 跑通单篇摘要。
</Note>
