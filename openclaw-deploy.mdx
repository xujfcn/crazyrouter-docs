...
- 安装器默认把 `crazyrouter` provider 指向 `https://cn.crazyrouter.com/v1`
...
<Steps>
  <Step title="创建专用 Crazyrouter token">
    在 Crazyrouter 后台创建一个专门给 OpenClaw 使用的 `sk-...` token。建议先只放行你打算在网关里使用的模型，例如 `claude-opus-4-8`、`gpt-5.5`、`gemini-3-pro`。
  </Step>
  <Step title="运行安装脚本">
    ```bash
...
| 高性价比备用 | `claude-sonnet-4-6` | 质量、稳定性和成本比较均衡 |
...
| `models.providers.crazyrouter.baseUrl` | OpenAI 兼容基址 | 通常保持 `https://cn.crazyrouter.com/v1` |
...
- 高频日常问答或机器人通知场景，可按成本回退到 `claude-sonnet-4-6` 或 `gemini-3-pro`
- 编码类工作流单独切到 `gpt-5.5`
...
默认保持安装器写入的值即可：OpenAI 兼容 provider 用 `https://cn.crazyrouter.com/v1`，Claude / MiniMax 原生兼容 provider 用 `https://cn.crazyrouter.com`。
