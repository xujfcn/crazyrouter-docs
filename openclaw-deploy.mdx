...
<Steps>
  <Step title="创建专用 Crazyrouter token">
    在 Crazyrouter 后台创建一个专门给 OpenClaw 使用的 `sk-...` token。建议先只放行你打算在网关里使用的模型，例如 `claude-sonnet-4-6`、`gpt-5.4`、`gemini-3-pro`。
  </Step>
  <Step title="运行安装脚本">
    ```bash
...
| 编码 / Agent | `gpt-5.4` | 2026 年 3 月 23 日已在生产环境实测成功，适合作为 OpenAI 兼容编码与 Agent 主基线 |
...
| `agents.defaults.model.primary` | 默认主模型 | 切换到 `gpt-5.4`、`claude-opus-4-7` 等 |
...
- 高频日常问答或机器人通知场景，优先尝试 `claude-sonnet-4-6` 或 `gemini-3-pro`
- 编码类工作流单独切到 `gpt-5.4`
