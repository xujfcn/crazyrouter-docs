...
<Note>
  API ルート: OpenAI 互換クライアントは通常 `https://cn.crazyrouter.com/v1` または `https://cn.crazyrouter.com/v1` を使います。Claude Code / Anthropic ネイティブクライアントは `/v1` を付けずにルートドメインを使います。詳しくは [API エンドポイント](/jp/api-endpoint) を参照してください。
</Note>
