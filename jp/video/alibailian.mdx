...
<Warning>
  Submitting `wan2.2-t2v-plus` to this page's path returns `invalid_request` in production, with guidance to use `/v1/video/generations`. For Wan text-to-video, see [Wan Video](/jp/video/wan) or [Video Generation Alias](/jp/video/openai-video-alias-create).
</Warning>
