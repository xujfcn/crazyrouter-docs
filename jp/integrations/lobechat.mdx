...
| Available models | Allow at least one verified OpenAI-compatible chat model such as `gpt-5.5` |
...
<Steps>
  <Step title="Create a dedicated LobeChat token">
    In the Crazyrouter dashboard, create a token named `lobechat`. For the first rollout, allow only the models you actually need, such as `gpt-5.5` and `claude-sonnet-4-6`.
  </Step>
  ...
  <Step title="Configure the OpenAI path">
    ...
    - `API Key`: your `sk-...`
    - `API Proxy URL`: `https://cn.crazyrouter.com/v1`
...
|----------|-------------------|-----|
| Default main chat model | `gpt-5.5` | Verified successfully in production on March 23, 2026, and suited for the main LobeChat baseline |
...
Recommended order: get `gpt-5.5` working first, then expand to `claude-sonnet-4-6` and `gemini-3-pro`.
...
- [ ] `API Proxy URL` is set to `https://cn.crazyrouter.com/v1`
...
|---------|--------------|-----|
| settings cannot be saved or validation fails | wrong API key or wrong proxy URL | recheck the `sk-...` value and `https://cn.crazyrouter.com/v1` |
...
| 404 | proxy URL was entered as the root domain or a full endpoint path | change it to `https://cn.crazyrouter.com/v1` |
...
| model looks selectable but requests fail | LobeChat cached an old model list or the default model is not valid | switch back to `gpt-5.5`, refresh, and reselect |
| chat works but streaming is unstable | client-version or model compatibility issue | use `gpt-5.5` as the baseline and upgrade LobeChat |
...
Use `https://cn.crazyrouter.com/v1`.
