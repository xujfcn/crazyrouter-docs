...
| `deepseek-reasoner` | DeepSeek | ✅ | ✅ | 推論 | 数学・複雑なロジック |
| `deepseek-chat` / `deepseek-v3` / `deepseek-v3.2` / `deepseek-r1` | DeepSeek | ✅ | ✅ | — | 一般的なテキストタスク |
| `MiniMax-M2.7` | MiniMax | ✅ | ✅ | ⚠️ text ブロック内に `<think>` タグで出力 | 長文コンテキスト・中国語タスク |
| `MiniMax-M2.5` | MiniMax | ✅ | ✅ | ⚠️ text ブロック内 | 一般会話 |
| `MiniMax-M2.1` | MiniMax | ✅ | ✅ | — | 思考なし、出力が最もクリーン |
...
> 検証は `https://cn.crazyrouter.com/v1/messages` に対し、`anthropic-version: 2023-06-01` ヘッダと標準の Anthropic Messages リクエストを用いて実施。
...
- **思考と課金**：思考対応モデル（`-thinking`、`-flash`、`reasoner`、`MiniMax-M2.5/2.7`）は出力トークンを多く消費します。Claude Code 専用 token に独立した予算上限を設定することを推奨します。
