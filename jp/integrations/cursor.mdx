...
<Steps>
  <Step title="Create a Cursor-specific token">
    In Crazyrouter, create a new token and name it something obvious like `cursor`. Start with a small allowlist such as `gpt-5.5` and `claude-sonnet-4-6`.
  </Step>
...
Recommended rollout order: get `gpt-5.5` working first, then test `claude-sonnet-4-6` and `gemini-3-pro`.
...
- [ ] `Override OpenAI Base URL` is set to `https://cn.crazyrouter.com/v1`
...
|---------|--------------|-----|
| `Verify` fails | wrong API key or wrong base URL | re-check the `sk-...` key and `https://cn.crazyrouter.com/v1` |
...
| `404` | base URL was set to the root domain or a full endpoint path | use exactly `https://cn.crazyrouter.com/v1` |
