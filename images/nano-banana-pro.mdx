...
- 本页只覆盖 `nano-banana-pro` / `gemini-3-pro-image-preview`
...
推荐在客户代码里使用 `nano-banana-pro`。如果你的系统已经直接写了 `gemini-3-pro-image-preview`，也可以继续通过同一个 `/v1/images/generations` 入口调用。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `model` | string | 是 | 推荐 `nano-banana-pro`；也兼容 `gemini-3-pro-image-preview` |
