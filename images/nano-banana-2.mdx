...
`nano-banana-2` 是 `gemini-3.1-flash-image-preview` 的公开别名。用户侧统一使用 OpenAI Images 兼容协议；服务端会把 `output_format`、`output_compression`、`resolution`、`aspect_ratio` 等统一参数转换成 Gemini / Vertex 原生请求。
...
推荐在客户代码里使用 `nano-banana-2`。如果你的系统已经直接写了 `gemini-3.1-flash-image-preview`，也可以继续通过同一个 `/v1/images/generations` 入口调用。

## 请求参数

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `model` | string | 是 | 推荐 `nano-banana-2`；也兼容 `gemini-3.1-flash-image-preview` |
