...
Model names, prices, billing modes, and endpoint support must align with https://crazyrouter.com/pricing and https://crazyrouter.com/api/pricing?lang=en.
If you need to confirm which models the current API key can call, use GET https://cn.crazyrouter.com/v1/models.
For OpenAI-compatible clients, use base_url https://cn.crazyrouter.com/v1.
For Claude Code / Anthropic-native clients, use root domain https://cn.crazyrouter.com and do not append /v1.
```
...
- Model names, prices, billing modes, and endpoint types come from the [Pricing page](https://crazyrouter.com/pricing) and `GET https://crazyrouter.com/api/pricing?lang=en`.
- Models callable by the current token come from `GET https://cn.crazyrouter.com/v1/models`.
- When generating API code, prefer `public_endpoint_types` from `/api/pricing`; fall back to `supported_endpoint_types` when needed.
