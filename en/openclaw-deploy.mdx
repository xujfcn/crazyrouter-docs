...
- the installer points the default `crazyrouter` provider at `https://cn.crazyrouter.com/v1`
...
<Note>
If you later edit OpenClaw provider addresses manually, check [API Endpoints](https://docs.crazyrouter.com/en/api-endpoint) first so you know whether this client should use the root domain or the `/v1` path.
</Note>
...
<Steps>
  <Step title="Create a dedicated Crazyrouter token">
    In Crazyrouter, create a dedicated `sk-...` token for OpenClaw. Start with a small model allowlist such as `claude-opus-4-8`, `gpt-5.5`, and `gemini-3-pro`.
  </Step>
...
| Cost-balanced fallback | `claude-sonnet-4-6` | Strong balance of quality, stability, and cost |
...
| `models.providers.crazyrouter.baseUrl` | OpenAI-compatible base URL | usually keep `https://cn.crazyrouter.com/v1` |
...
- keep routine bot traffic on `claude-sonnet-4-6` or move selected tasks to `gemini-3-pro` when cost matters more
...
Leave the installer defaults unless you are intentionally changing provider modes: `https://cn.crazyrouter.com/v1` for the OpenAI-compatible provider, and `https://cn.crazyrouter.com` for Claude / MiniMax native-compatible providers.
