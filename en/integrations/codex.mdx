...
- recommended default model: `gpt-5.3-codex`
...
- coding-first workflows built around models such as `gpt-5.3-codex`
...
<Note>
Crazyrouter supports `/v1/responses`, so this Codex CLI integration can use the Responses API directly.
</Note>
...
| Allowed models | Allow at least one coding-friendly model such as `gpt-5.3-codex` |
...
|----------|-------------------|-----|
| default coding driver | `gpt-5.3-codex` | strongest fit for code generation, edits, and agent-style workflows |
| balanced daily work | `gpt-5.2` | better balance of capability and cost |
| stable fallback baseline | `gpt-4.1` | good for first-pass validation |
| cheaper high-frequency runs | `gpt-4.1-mini` | useful for lighter prompts and repeated small tests |

Recommended rollout: get `gpt-5.3-codex` working first. If cost matters more, fall back to `gpt-5.2` or `gpt-4.1`.
...
- keep `gpt-5.3-codex` as the main driver, but use `gpt-5.2` for lighter daily work
...
<Note>
If you want the smoothest terminal agent workflow on the OpenAI / Responses API path, Codex is one of the most important Crazyrouter integrations to configure well.
</Note>
