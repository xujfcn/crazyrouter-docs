...
- recommended default models: `claude-sonnet-4-6` or `gpt-5.3-codex`
...
| stronger coding execution | `gpt-5.3-codex` | better fit for complex edits and agent-style execution |
| stable compatibility baseline | `gpt-4.1` | useful for first-pass validation |
| cheaper trial runs | `gpt-4.1-mini` | better for light tasks and frequent small requests |

Recommended rollout: get the workflow stable on `claude-sonnet-4-6` first. If raw code-edit performance matters more, move to `gpt-5.3-codex`.
...
- use `claude-sonnet-4-6` or `gpt-5.3-codex` for complex refactors
- move repetitive light tasks to `gpt-4.1-mini`
