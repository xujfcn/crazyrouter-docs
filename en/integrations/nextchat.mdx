...
<Steps>
  <Step title="Create a dedicated NextChat token">
    In the Crazyrouter dashboard, create a token named `nextchat`. For the first rollout, allow only baseline models such as `gpt-5.5` and `claude-sonnet-4-6`.
  </Step>
...
services:
  nextchat:
    ...
    environment:
      ...
      - CUSTOM_MODELS=+gpt-5.5,+claude-sonnet-4-6,+gemini-3-pro
      - HIDE_USER_API_KEY=1
```
...
| `CUSTOM_MODELS` | `+gpt-5.5,+claude-sonnet-4-6,+gemini-3-pro` | adds selectable models in the UI |
