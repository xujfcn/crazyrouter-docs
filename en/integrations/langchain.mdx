...
<Steps>
  <Step title="Step 1: Create a LangChain-specific Crazyrouter token">
    ...
    Add stronger models such as `gpt-5.2` only after the basic route is already stable.
  </Step>
...
|----------|-------------------|-----|
| first-pass validation | `gpt-4.1` | easiest baseline for proving that LangChain and Crazyrouter are connected |
| high-frequency light tasks | `gpt-4.1-mini` | lower cost for simpler chains |
| stronger reasoning and orchestration | `gpt-5.2` | better for more complex flows |
...
| `model not found` | wrong model name or the token does not allow it | switch back to `gpt-4.1` and check the allowlist |
