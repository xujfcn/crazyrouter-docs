...
| Available models | Allow at least one chat model such as `gpt-4.1` or `gpt-4.1-mini` |
...
<Steps>
  ...
  <Step title="Fetch and add models">
    Open `Manage`, fetch the model list, and add the models you want to use. Seeing a model in the popup does not always mean it is already enabled in the provider list, so you may still need to click `+` to add it. For first-run validation, start with only one baseline model: `gpt-4.1`.
  </Step>
  <Step title="Enable the provider and send the first prompt">
    Turn on the provider toggle, go back to the chat screen, pick `gpt-4.1`, and send a simple prompt such as `Reply only OK`. If you get a normal reply, the setup is working.
  </Step>
</Steps>
...
|----------|-------------------|-----|
| Default chat model | `gpt-4.1` | Stable and reliable for most desktop chat workflows |
| Cheap everyday usage | `gpt-4.1-mini` | Fast and inexpensive for frequent prompts |
| Stronger general-purpose sessions | `gpt-4o` | Good for richer multi-turn interaction |
| Higher-quality code / long-form help | `claude-sonnet-4-6` | Strong reasoning, writing, and code explanation |
| Budget fallback | `gemini-2.5-flash` | Useful for lightweight tasks and quick experiments |

Recommended order: get `gpt-4.1` working first, then add stronger or cheaper models gradually.
...
| `Check` still fails even though the URL and key look right | Cherry Studio often validates against the last conversational model currently present in the provider list, and that model may be invalid | keep only `gpt-4.1`, remove suspicious models, and run `Check` again |
...
| connection works but chat still fails | the added model is incompatible or unavailable | keep only `gpt-4.1` for baseline testing |
| streaming behaves oddly | model-level or client-version compatibility issue | switch back to `gpt-4.1` and upgrade Cherry Studio |
...
- Use `gpt-4.1-mini` for frequent low-cost chat, then switch to `gpt-4.1` or `claude-sonnet-4-6` for harder tasks
