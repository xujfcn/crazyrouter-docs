...
- use `gpt-4.1` as the first validation model
...
|------|----------|------|
| First connection test | `gpt-4.1` | Best first baseline for plugin-to-Crazyrouter validation |
| Literature translation | `gpt-4.1-mini` | Lower cost for repeated translation work |
...
- [ ] `gpt-4.1` was used as the first model
...
| `model not found` | The model name is wrong | Switch back to `gpt-4.1` and do a minimal validation |
...
<Note>
If your goal is "make Zotero work with Crazyrouter first", the key is not choosing the fanciest plugin. It is confirming that your plugin truly supports a custom OpenAI-compatible endpoint, and then validating a single-paper summary with `gpt-4.1`.
</Note>
