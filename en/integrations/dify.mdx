...
<Tip>
The exact UI label can move between `Model Provider`, `Providers`, localized labels, or plugin-related entry points across Dify versions. Keep the first-pass validation flow unchanged: save the provider first, add only one `LLM` model `gpt-4.1`, and validate with a minimal Chat app before adding anything else.
</Tip>
...
| Available models | Allow at least one chat model such as `gpt-4.1` |
...
<Steps>
  <Step title="Create a dedicated Dify token">
    In the Crazyrouter dashboard, create a token named `dify`. For the first pass, allow `gpt-4.1` and one embedding model such as `text-embedding-3-large`.
  </Step>
...
|----------|-------------------|-----|
| Default workflow chat model | `gpt-4.1` | Stable for most Dify app flows |
| Lower-cost testing / batch runs | `gpt-4.1-mini` | Better for frequent workflow experiments |
| Stronger general-purpose workflows | `gpt-4o` | Better for heavier context and interaction |
...
| provider saves but the app still fails | wrong model name or bad app-level parameter choices | fall back to `gpt-4.1` for baseline validation |
...
| some advanced models behave inconsistently in Dify | model-level parameter compatibility differs across nodes | validate on `gpt-4.1` first, then swap in stronger models gradually |

## Performance and Cost Tips

- Use `gpt-4.1-mini` for workflow testing first, then move to more expensive models after validation
...
Because Dify issues can come from the provider layer, model type, app configuration, or knowledge-base chain. Validating a single `gpt-4.1` chat app first makes isolation much easier.
