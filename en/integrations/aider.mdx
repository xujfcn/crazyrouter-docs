...
- recommended default model string: `openai/gpt-5.3-codex`
...
One important accuracy detail: in Aider's official OpenAI-compatible docs, model names are typically written as `openai/<model-name>`. So Crazyrouter's `gpt-5.3-codex` should usually be entered in Aider as:
...
<Warning>
This is one of the most important corrections in this doc upgrade. In your Crazyrouter token allowlist, keep the raw model id such as `gpt-5.3-codex`. But in Aider commands and `~/.aider.conf.yml`, prefer the `openai/...` model form.
</Warning>
...
<Steps>
  <Step title="Create an Aider-specific token">
    In Crazyrouter, create a token named something like `aider`. Start by allowing only models such as `gpt-5.3-codex`, `gpt-4.1`, and `claude-sonnet-4-6`.
...
|----------|--------------------------------|-----------------------|-----|
| default coding driver | `openai/gpt-5.3-codex` | `gpt-5.3-codex` | best fit for code edits, diff reasoning, and repeated repair loops |
| stable baseline | `openai/gpt-4.1` | `gpt-4.1` | useful for first-pass compatibility checks |
...
| cheaper high-frequency tasks | `openai/gpt-4.1-mini` | `gpt-4.1-mini` | useful for lighter prompts and routine adjustments |

Recommended rollout: start with `openai/gpt-5.3-codex`. If cost matters more or you are just validating the path, use `openai/gpt-4.1` first.
...
| `model not found` | wrong model string or missing the `openai/` prefix in Aider | switch back to `openai/gpt-5.3-codex`, `openai/gpt-4.1`, or another confirmed `openai/...` model string |
| Aider launches but performs poorly | the model is too weak or the context is too noisy | return to `gpt-5.3-codex` and reduce the context files |
...
- keep `openai/gpt-5.3-codex` as the main model and move lighter work to `openai/gpt-4.1` or `openai/gpt-4.1-mini`
...
### Why should the model name usually be `openai/gpt-5.3-codex` instead of just `gpt-5.3-codex`?
...
Start with `openai/gpt-5.3-codex`.
