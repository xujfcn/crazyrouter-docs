...
> Tested against `https://cn.crazyrouter.com/v1/messages` with `anthropic-version: 2023-06-01` using a standard Anthropic Messages payload.
...
- **MiniMax family**: the only models whose pricing entry explicitly advertises `anthropic` in `supported_endpoint_types`, so the protocol layer is the most direct fit. However, `M2.5` / `M2.7` emit their reasoning as a regular text block **wrapped in `<think>...</think>` tags** — Claude Code will display them verbatim. Two ways to avoid this:
  1. Use `MiniMax-M2.1` (no thinking trace).
  2. Add a system prompt: "Do not output `<think>` tags or your reasoning, only return the final answer."
