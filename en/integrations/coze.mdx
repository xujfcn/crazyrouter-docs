...
|----------|----------|------|
| First connection test | `gpt-4.1` | Best starting point for validating the Coze-to-Crazyrouter path |
| Lower-cost test runs | `gpt-4.1-mini` | Cheaper for plugin and workflow debugging |
...
- [ ] Validated the smallest request body with `gpt-4.1`
...
| `model not found` | Model name is wrong | Switch back to `gpt-4.1` and revalidate |
