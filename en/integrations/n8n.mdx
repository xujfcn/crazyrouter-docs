...
<Steps>
  <Step title="Create a dedicated n8n token">
    In the Crazyrouter dashboard, create a token named `n8n`. For the first rollout, allow only `gpt-4.1` and `gpt-4.1-mini`.
  </Step>
  ...
  <Step title="Build a minimal workflow">
    Create a workflow such as `Manual Trigger` → `OpenAI Chat Model` → `Output`, and set the model to `gpt-4.1`.
  </Step>
...
|----------|-------------------|-----|
| Default workflow model | `gpt-4.1` | Stable for most automation flows |
| Lower-cost bulk jobs | `gpt-4.1-mini` | Better for high-frequency workflow runs |
| More complex agent flows | `gpt-4o` | Better for multi-step reasoning and richer inputs |
| Retrieval / vector-related flows | `text-embedding-3-large` | Good for later vector or search-enhanced pipelines |

Recommended order: get `gpt-4.1` working in a minimal flow, then expand to agents, tools, and batch jobs.
...
| batch jobs are slow or expensive | a premium model is being used for large row counts | move bulk tasks to `gpt-4.1-mini` first |

## Performance and Cost Tips

- Use `gpt-4.1-mini` or `gpt-4.1` as the default automation baseline
