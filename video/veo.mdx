...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `model` | string | 是 | `veo-3.1-fast` 或 `veo-3.1` |
...
```bash cURL
curl "https://cn.crazyrouter.com/v1/video/query?id=veo_task_abc123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
<Note>
  Google 官方文档还描述了 Veo 3.1 的 `reference asset images`、以及更细的 provider-native 能力边界。当前 Crazyrouter 对客公开页先只承诺 `veo-3.1-fast` / `veo-3.1` 下的文生视频与单图生视频；如果后续开放更多 Veo 一级能力，会单独补文档与验证记录。
</Note>
