...
| `kling-v3` | Kling Omni 视频 | 单镜头 Omni 视频 | `Beta` | `mode + duration + video input` |
...
```bash cURL
curl -X POST https://cn.crazyrouter.com/kling/v1/videos/text2video \
...
response = requests.post(
    "https://cn.crazyrouter.com/kling/v1/videos/text2video",
    headers={
...
```bash cURL
curl -X POST https://cn.crazyrouter.com/kling/v1/videos/multi-image2video \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
    "model_name": "kling-v3",
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `model_name` | string | 是 | 仅支持 `kling-v3`、`kling-v3` |
...
```bash cURL
curl -X POST https://cn.crazyrouter.com/kling/v1/videos/omni-video \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
    "model_name": "kling-v3",
...
<Note>
  `kling-v3` 当前按官方能力仅开放单镜头 Omni 生成；`kling-v3` 支持更完整的 Omni 引用能力。Omni 路由推荐使用官方 `element_list`，同时兼容 `kling_elements` 别名。若要查询任务，请使用与提交路径同族的 `GET /kling/v1/videos/omni-video/{task_id}`。
</Note>
