...
<Note>
  The more usable path right now is [Nano Banana 2](/ru/images/nano-banana-2), which now points to native Gemini `gemini-3.1-flash-image-preview:generateContent`.
  [Nano Banana](/ru/images/nano-banana) now keeps only the controlled-test note for `gemini-2.5-flash-image:generateContent`, not a stable production recommendation.
</Note>
