...
<Steps>
  <Step title="Create a dedicated n8n token">
    In the Crazyrouter dashboard, create a token named `n8n`. For the first rollout, allow only `gpt-5.5` and `claude-sonnet-4-6`.
  </Step>
  ...
  <Step title="Build a minimal workflow">
    Create a workflow such as `Manual Trigger` → `OpenAI Chat Model` → `Output`, and set the model to `gpt-5.5`.
  </Step>
...
|----------|-------------------|-----|
| Default workflow model | `gpt-5.5` | Verified successfully in production on March 23, 2026, and suited for the main n8n baseline |
...
- [ ] node-level `Base URL` is set to `https://cn.crazyrouter.com/v1`
...
| 404 | `Base URL` was entered as the root domain or a full endpoint path | change it to `https://cn.crazyrouter.com/v1` |
