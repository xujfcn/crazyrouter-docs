...
<Note>
Official Dify docs treat a custom `API Base URL` as an optional field for proxy or compatible-upstream scenarios. For Crazyrouter, that is exactly the intended pattern, so use `https://cn.crazyrouter.com/v1` as the OpenAI-compatible upstream. Also, model-provider configuration is usually limited to workspace admins or owners; if you cannot see the entry, check your role first.
</Note>
...
- [ ] `API Base URL` is set to `https://cn.crazyrouter.com/v1`
...
| 404 | `API Base URL` was entered as the root domain or a full endpoint path | change it to `https://cn.crazyrouter.com/v1` |
