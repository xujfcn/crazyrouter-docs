...
| `deepseek-v4-pro` | DeepSeek | ✅ | ✅ | reasoning | Математика, сложная логика |
| `deepseek-v4-pro` / `deepseek-v4-pro` / `deepseek-v4-pro` / `deepseek-v4-pro` | DeepSeek | ✅ | ✅ | — | Общие текстовые задачи |
| `MiniMax-M2.7` | MiniMax | ✅ | ✅ | ⚠️ внутри text-блока как теги `<think>` | Длинный контекст, китайские задачи |
| `MiniMax-M2.7` | MiniMax | ✅ | ✅ | ⚠️ внутри text-блока | Общий чат |
| `MiniMax-M2.7` | MiniMax | ✅ | ✅ | — | Без thinking, чистейший вывод |
...
- **Семейство MiniMax**: единственные модели, у которых в записи прайс-листа поле `supported_endpoint_types` явно содержит `anthropic`, поэтому уровень протокола подходит лучше всего. Однако `M2.5` / `M2.7` отдают рассуждения в обычном text-блоке **обёрнутыми тегами `<think>...</think>`** — Claude Code показывает их как есть. Способы избежать:
  1. Использовать `MiniMax-M2.7` (без рассуждений).
...
- **Thinking и тарификация**: модели с thinking (`-thinking`, `-flash`, `reasoner`, `MiniMax-M2.7`) расходуют дополнительные output-токены. Установите отдельный лимит бюджета на токен для Claude Code.
