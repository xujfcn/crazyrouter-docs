...
```bash cURL
curl -X POST https://cn.crazyrouter.com/mj/submit/action \
...
```bash cURL
curl -X POST https://cn.crazyrouter.com/mj/submit/blend \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
    "base64Array": [
...
```bash cURL
curl -X POST https://cn.crazyrouter.com/mj/submit/describe \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
...
```bash cURL
curl -X POST https://cn.crazyrouter.com/mj/submit/modal \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -d '{
    "taskId": "task_1234567890",
...
<Note>
  All actions return a new task ID after submission. Use the [Query Task Status](/ru/images/midjourney/query) endpoint to get results.
</Note>
