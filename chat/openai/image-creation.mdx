...
<Note>
  如果你只是手工试用，`gpt-4o-image` 的 Chat 路径仍然可以观察结果；如果你要做前后端自动化、结果存储或图片后处理，请直接用 [`GPT Image 文档`](/images/gpt-image)、[`Grok 图片模型`](/images/grok) 或 [`Nano Banana 2`](/images/nano-banana-2)。图片类长耗时请求优先使用 `https://cn.crazyrouter.com/v1`。
</Note>
