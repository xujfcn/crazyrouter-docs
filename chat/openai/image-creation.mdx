...
<Note>
  如果你只是手工试用，`gpt-4o-image` 的 Chat 路径仍然可以观察结果；如果你要做前后端自动化、结果存储或图片后处理，请直接用 [`GPT Image 文档`](/images/gpt-image)。如果你的需求是参考图编辑或多图输入，当前优先改看 [`Nano Banana 2`](/images/nano-banana-2) 的 Gemini 原生路径；不要再把 Nano Banana family 的 `/v1/images/generations` 当成稳定推荐方案。
</Note>
